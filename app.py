...
if enable_country_range:
    countries = {
        k: countries[k]
        for k in list(countries.keys())[country_range[0] : country_range[1]]
    }
...
class FlagQuiz:
    def __init__(self, root):
        ...
        self.all_country_codes = list(countries.keys())
    ...
    def check_answer(self):
        ...
        if user_input == correct_answer:
            ...
            self.feedback_label.config(
                text=f"Incorrect. It was {self.correct_name}.", fg="red"
            )
            self.total_questions += 1
            self.status_label.config(text=f"Score: {self.score}/{self.total_questions}")
